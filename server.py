...
def _set_sqlite_prama(dbapi_connection , connection_record):
...
def get_user_ascending_id():
    users = User.query.all()
    all_user_ll = linked_list.LinkedList()
    
    for user in users:
        all_user_ll.insert_ending(
            {
                "id" : user.id,
                "name" : user.name,
                "email" : user.email,
...
def delete_user(user_id):
    ...
    db.session.commit()
    return jsonify("id number {} deleted".format(user.id)) , 200
    
@app.route("/blog_post/<user_id>" , methods=["POST"])
def create_blog_posts(user_id):
    data = request.get_json()
    ...
    db.session.commit()
    return jsonify({"message" : "id {} has created successfully".format(user_id)})

@app.route("/blog_post/<blog_id>" , methods=["GET"])
def get_all_blog_posts(blog_id):
    blog_posts=  BlogPost.query.all()
    random.shuffle(blog_posts)
    bst = binary_search.BinarySearchTree()
    ...
    if not post:
        return jsonify({"message" : "Post not found, please add or try another methods"})
    return jsonify(post)
@app.route("/blog_post/numberic_body" , methods=["GET"])
def get_numberic_body():
    blog_posts = BlogPost.query.all()
...
def delete_blog_posts():
    ...
    for _ in range(10):
        ...
        db.session.commit()
    return jsonify({"message" : "delete the last 10 post successfully"})
